///<reference path="LockInfo.d.ts"/>
///<reference path="LockResult.d.ts"/>
///<reference path="LockLevel.d.ts"/>
///<reference path="RefreshLockResult.d.ts"/>
...
declare module ITHit.WebDAV.Server.Class2 {
	...
	* #####
	*
	* @description <br><p>  This interface provides the means for locking the hierarchy item, updating lock timeout and accessing the  list of applied locks. To create WebDAV Class 2 compliant server you must implement this interface on  your file and folder items. </p><p>  When this interface is implemented on an item the server reports Class 2 compliance, returning DAV: 1, 2, 3  header in response to the OPTIONS request. Note that while most WebDAV clients never lock folder items,  you must still add this interface on folders, as soon as WebDAV clients submit OPTIONS request against folder  items when discovering server compliance. </p><p>  When a WebDAV client requires to protect an item from concurrent updates, it locks the item (usually file),  submitting lock request to server. The server generates the new lock-token, marks the item as locked and returns  the new lock-token to the client. The WebDAV client application keeps the lock-token and when it requires to  perform any updates, it supplies the lock-token with the request. When the server receives the update request,  it verifies that the lock token belongs to the item that is being updated and performs modifications. </p>
	*/
	export interface ILock
	{
		...
		* @description <br><param> This property must return all locks for the item including deep locks on any of the parent folders. </param>
		* @returns .
		*/
		getActiveLocks() : Promise<ITHit.WebDAV.Server.Class2.LockInfo[]>;
		/**
		* Locks this item.
		* #####
		*
		...
		* @throws [DavException]{@link ITHit.WebDAV.Server.DavException} In other cases.
		* @description <br>This method is called when item is being locked by WebDAV client. In your implementation you must do the following: 1. 2. <description>Generate the new lock-token, usually GUID.</description>3. <description>Save information about the lock in a storage.</description>4. <description>Associate the lock with the item in the repository.</description>5. <description>Return the lock-token to the Engine.</description> Optionally in in this method you can modify the lock timeout requested by client. For example instead of infinity  lock you can set lock for some limited time. You must return both lock-token and lock timeout via [LockResult](ITHit.WebDAV.Server.Class2.LockResult)   return value, the engine than sends the lock-token and timeout values back to WebDAV client.
		...
		*/
		lock(level: ITHit.WebDAV.Server.Class2.LockLevel, isDeep: boolean, requestedTimeOut: any, owner: string) : Promise<ITHit.WebDAV.Server.Class2.LockResult>;
		/**
		...
		* @throws [DavException]{@link ITHit.WebDAV.Server.DavException} In other cases.
		* @description <br>This method is called when WebDAV client wants to modify (usually prolong) timeout for the previously  set lock. In this method implementation you can update the lock timeout. Note that you can ignore the requested  timout and set timeout that is different from the one requested by client.
		...
		*/
		refreshLock(token: string, requestedTimeOut: any) : Promise<ITHit.WebDAV.Server.Class2.RefreshLockResult>;
		/**
		...
		* @throws [MultistatusException]{@link ITHit.WebDAV.Server.MultistatusException} Errors has occured during processing of the subtree.
		* @throws [DavException]{@link ITHit.WebDAV.Server.DavException} In other cases.
		* @description <br><p>  If this lock included more than one hierarchy item, the lock is removed from all items included in the lock. </p>
		* @returns .
		*/
		unlock(lockToken: string) : any;
	}
}
