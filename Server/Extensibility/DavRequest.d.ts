declare module ITHit.WebDAV.Server.Extensibility {
	...
	*/
	export class DavRequest
	{
		...
		* #####
		*
		* @description <br><c>ClientLockTokens</c> property provides access to the list of lock tokens  submitted by the client. These lock tokens were generated during the call to your  [ILock.lock](ITHit.WebDAV.Server.Class2.ILock#lock)  method implementation, associated with the item and returned to client.  When WebDAV client is modifying any server item it  sends back to server the list of lock tokens. In your WebDAV server Class 2  implementation before modifying any locked items you must check if WebDAV  client provided necessary lock token.
		*/
		public clientLockTokens: string[];
	}
}
